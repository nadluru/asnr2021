...
distancedf = read.csv(paste0(csvroot, 'DistancesRegionalSelect_ASNR2021.csv')) %>% mutate(Consensus.Diagnosis = Consensus.Diagnosis %>% as.factor %>% fct_relevel('AD', after = Inf)) %>% 
  filter(!(SubjectID %in% c('ADCP_2066'))) # brain masking left eye tissue
orderInfoRegions = distancedf %>% pivot_wider(names_from = Consensus.Diagnosis, values_from = MeanDistance) %>% group_by(RegionalName) %>% summarise(MmC = mean(MCI, na.rm = T) - mean(CU, na.rm = T), AmC = mean(AD, na.rm = T) - mean(CU, na.rm = T)) %>% ungroup()
...
p = mdls %>%
  filter(str_detect(term, 'Consensus')) %>%
